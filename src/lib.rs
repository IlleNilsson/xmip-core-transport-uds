...
//! one ISO-TP message, segmented and reassembled there. The two directed
//! buses of one session are can-bus's loopback bus, so a tester and an ECU
//! round-trip in process with no hardware, which is what
//! [`UdsTransport::loopback`] stands up (ADR-0051).
//!
...
mod tests {
    ...
    fn pair() -> (UdsTransport, UdsTransport) {
        ...
        let tester = IsoTpTransport::new(Arc::clone(&to_ecu), Arc::clone(&to_tester), TESTER_ID)
            .timing_out_after(quick);
        let ecu = IsoTpTransport::new(to_tester, to_ecu, ECU_ID).timing_out_after(quick);
